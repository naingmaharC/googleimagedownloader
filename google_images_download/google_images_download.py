...
args_list = ["keywords", "keywords_from_file", "prefix_keywords", "suffix_keywords",
             ...
             "offset", "no_download","save_source","silent_mode","ignore_urls","save_zip","desktop","delete","csv"]
...
def user_input():
    ...
    if object_check['config_file'] != '':
        ...
        parser.add_argument('-csv', '--save_csv', default=False,help="output csv file",action="store_true")
        parser.add_argument('-zip', '--save_zip', default=False,help="output zip file",action="store_true")
        parser.add_argument('-desktop', '--desktop', default=False,help="Chage output directory to desktop",action="store_true")
        parser.add_argument('-del', '--delete', default=False,help="delete download directory",action="store_true")
...
class googleimagesdownload:
    ...
    def build_search_url(self,search_term,params,url,similar_images,specific_site,safe_search):
        ...
        #     url = 'https://www.bing.com/images/search?q=' + keywordem + '&FORM=HDRSC2'
        # elif specific_site:
        ...
        keywordem = self.similar_images(similar_images)
        url = 'https://www.bing.com/images/async?q=' + urllib.parse.quote_plus("hello")
        print(url)
        return url
    ...
    def download_image_thumbnail(self,image_url,main_directory,dir_name,return_image_name,print_urls,socket_timeout,print_size,no_download,save_source,img_src,ignore_urls,save_zip):
        if print_urls or no_download:
            print("Image URL: " + image_url)
        if save_zip == "":
            print("Save Zip Start Working at  Download Image thumbnails")
        if no_download:
    ...
    # Download Images
    def download_image(self,image_url,image_format,main_directory,dir_name,count,print_urls,socket_timeout,prefix,print_size,no_numbering,no_download,save_source,img_src,silent_mode,thumbnail_only,format,ignore_urls,save_zip):
        image_name=""
        if not silent_mode:
            if print_urls or no_download:
                print("Image URL: " + image_url)
        ...
        if save_zip:
            return "Testing","Save as zip",None,image_url
        try:
            req = Request(image_url, headers={
                "User-Agent": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"})
            try:
        ...
        download_Image_info.append({
            "csvPath":main_directory + "/" + dir_name + "/",
            "name":dir_name,
            ...
            if object == "no_links":
                ...
                if download_status == "success":
                    ...
                    if arguments['thumbnail'] or arguments["thumbnail_only"]:
                        download_status, download_message_thumbnail = self.download_image_thumbnail(object['image_thumbnail_url'],main_directory,dir_name,return_image_name,arguments['print_urls'],arguments['socket_timeout'],arguments['print_size'],arguments['no_download'],arguments['save_source'],arguments['save_zip'],object['image_source'],arguments['ignore_urls'])
                        if not arguments["silent_mode"]:
    ...
    def download_executor(self,arguments):
        ...
        if arguments['output_directory']:
            ...
            main_directory = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
        else:
            main_directory = "downloads"
...
def csvController():
    ...
    csvPath = ""
    data = []
    for row in download_Image_info:
       
        if len(download_Image_info) == download_Image_info.index(row)+1:
            print(f"{len(download_Image_info)} , { download_Image_info.index(row)}")
            generateCsv(current_name,row["csvPath"],data)

        if row["name"] != current_name: 

            if current_name:
                generateCsv(current_name,csvPath,data)
                print(data)
                data = []
...
def generateCsv(name,csvPath,result):
        with open(csvPath+"/" +name+ ".csv", "w", newline='', encoding="utf-8") as csv_report:
            writer = csv.writer(csv_report)
            writer.writerow(['name',"Image Name","URL","Path","Status"])
...
def findGIDDir(mainPath,isDesktop):
    
    if mainPath:
        workingPath = arguments["output_directory"]
    elif isDesktop:
        workingPath = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
    else:
        workingPath = "downloads"

    actionDirList = []
    for subDirName in os.listdir(workingPath):
        subDir = os.path.join(workingPath, subDirName)
        if (subDirName.find('_gid.zip') != -1): 
            actionDirList.append(subDirName)
            print(f"No. {len(actionDirList)}  : {subDirName}")
        if os.path.isdir(subDir):
            for file in os.listdir(subDir):
                if file.endswith(".gid"):
                    ...
                    print(f"No. {len(actionDirList)}  : {subDirName}")
    try:
        directoryNo = input("Folder No For Delete: ")
        directoryNo = int(directoryNo)-1
        if (actionDirList[directoryNo].find('_gid.zip') != -1): 
            os.remove(workingPath+"\\"+actionDirList[directoryNo])
            print(f"Sucessfully delete {actionDirList[directoryNo]}")
        else:
            shutil.rmtree(workingPath+"\\"+actionDirList[directoryNo])
            print(f"Sucessfully delete {actionDirList[directoryNo]} folder")
    except:
        print("That folder is not exisit")
    
    sys.exit()
    


def zipController(keywords,mainPath,isDesktop):
    
    if not keywords:
        print("Keyword is empty")
    
    dirList = keywords.split(",")
    for dirName in dirList:
       
        if mainPath:
            workingPath = arguments["output_directory"]+"/"+dirName
        elif isDesktop:
            workingPath = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')+"/"+dirName
        else:
            workingPath = "downloads/"+dirName
        zipf = zipfile.ZipFile(workingPath+'_gid.zip', 'w', zipfile.ZIP_DEFLATED)
        zipdir(workingPath, zipf)
        zipf.close()

def identifyAsGID(keywords,mainPath,isDesktop):
    if not keywords:
        print("Keyword is empty")
    ...
    for dirName in dirList:
        ...
        if mainPath:
            ...
            workingPath = "downloads/"+dirName
        open(workingPath+"/.gid","a").close()
        # os.rename(workingPath,workingPath+"_gid")
...
def main():
    ...
    for arguments in records:
        if arguments['delete']:
            findGIDDir(arguments["output_directory"],arguments["desktop"])
        ...
        if arguments["save_zip"]:
            zipController(arguments["keywords"],arguments["output_directory"],arguments["desktop"])
            # print("I am zip file"+arguments["keywords"]+arguments["output_directory"])
        # print(download_Image_info)
        if arguments["save_csv"]:
            csvController()
        identifyAsGID(arguments["keywords"],arguments["output_directory"],arguments["desktop"])
        if not arguments["silent_mode"]:
